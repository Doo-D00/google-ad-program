...
## 8. 지금 상태
정적 웹앱 뼈대 완성. 화면·설정·커서 삽입·미리보기·게시 흐름까지 구현되어 있고,
실제 브라우저에서 버튼 삽입과 미리보기는 검증했다.
**아직 실제 키로 검증하지 않은 것: Claude 생성, Gemini 생성, GitHub 업로드, Blogger 게시(W1~W3).**
다음 할 일은 `SETUP.md` 대로 키/OAuth 를 준비한 뒤 W1부터 순서대로 확인하는 것.
